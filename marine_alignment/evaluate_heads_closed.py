"""
evaluate_heads_closed.py — Test-Set Evaluation of the Closed (Joint) Model
=========================================================================
All data-split decisions are delegated to dataset.py.
This script only handles:
  1. Loading the trained Closed (Joint) checkpoint.
...
REPORT_PATH = os.path.join(
    ...
    "test_evaluation_report_closed.json"
)
...
def main(args):
    ...
    if not os.path.exists(CHECKPOINT_PATH_CLOSED):
        print(f"\n[ERROR] No checkpoint at:\n  {CHECKPOINT_PATH_CLOSED}")
        print("Run train.py first.\n")
        sys.exit(1)

    # ── Load trained pipeline ─────────────────────────────────────────────────
    pipeline = MarineImageBindPipeline().to(device)
    ckpt = torch.load(CHECKPOINT_PATH_CLOSED, map_location=device, weights_only=False)
